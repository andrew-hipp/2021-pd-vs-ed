...
There are a few ways we could skin this cat.
  * __Prioritize spp by their evolutionary distinctiveness globally.__ This means you would take the whole phylogeny of all spp (extant spp only, presumably, b/c I think from a conservation standpoint, extinct spp exist only via extant spp, unless perhaps if we think of history as having moral standing aside from its endpoints in the current day, which I think I do, but I haven't got this worked out yet in my head) and calculate evolutionary distinctiveness in some way for all tips (we'll get to metrics in a moment). This is the approach taken in [Larkin et al. 2015](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0156973). 
